...
class BrandListPage extends StatefulWidget {
  const BrandListPage({
    ...
    this.take = 48,
    this.emptyMessage = 'No brands found',
  });

  final String title;
  final BrandListFetcher fetcher;
  final void Function(BrandEntity brand) onBrandTap;
  final int take;
...
class _BrandListPageState extends State<BrandListPage> {
  ...
  List<BrandEntity> get _filtered {
    final q = _query.trim().toLowerCase();
    return _items.where((b) {
      if (_categoryFilter != null &&
          (b.categoryName ?? '').trim() != _categoryFilter) {
        return false;
      }
  ...
  String _subtitleOf(BrandEntity brand) {
    final bio = brand.bio?.trim();
    if (bio != null && bio.isNotEmpty) return bio;
    final category = brand.categoryName?.trim();
  ...
  Widget _buildBody() {
    ...
    if (items.isEmpty) {
      ...
      return Center(
        child: Text(
          hasFilters ? 'No brands match your search' : widget.emptyMessage,
          style: GoogleFonts.roboto(
            fontSize: 14,
