...
class HomeController extends GetxController {
  ...
  /// IDs currently mid-toggle (favourite request in flight). Used to
  /// disable/spin the favourite button for that one campaign only, and
  /// to guard against double-taps firing two overlapping requests.
  ...
  // ---------------------------------------------------------------
  // Header (from AuthController.currentUser + ProfileController)
  // ---------------------------------------------------------------
  ...
  /// Creator level name from `/creator/profile` progress, e.g. "Elite".
  String get levelName => _profileController?.levelName ?? '';

  /// Creator level code from `/creator/profile` progress, e.g. "elite".
  String get levelCode => _profileController?.levelCode ?? '';

  /// Avatar from `/auth/me` (resolved absolute URL).
  String? get avatarUrl {
    if (Get.isRegistered<AuthController>()) {
      return Get.find<AuthController>().avatarUrl;
    }
    return resolveImageUrl(currentUser?.avatarUrl);
  }
  ...
  /// POST /creator/marketplace/{id}/favourite or /unfavourite based
  /// on the local favourite state. The API returns the resulting
  /// `isFavourite` in `data`, which we mirror locally.
  Future<void> toggleFavouriteCampaign(String campaignId) async {
    ...
    final result = await _toggleFavouriteCampaignUseCase(
      ToggleFavouriteCampaignParams(
        ...
        currentlyFavourite: _isCampaignFavourite(campaignId),
      ),
    );
  ...
  bool _isCampaignFavourite(String campaignId) {
    if (favouriteCampaigns.any((f) => f.campaign.id == campaignId)) {
      return true;
    }
    for (final c in campaigns) {
      if (c.id == campaignId) return c.isFavourite;
    }
    for (final c in popularCampaigns) {
      if (c.id == campaignId) return c.isFavourite;
    }
    for (final r in recentCampaigns) {
      if (r.campaign.id == campaignId) return r.campaign.isFavourite;
    }
    for (final r in marketplaceRecentCampaigns) {
      if (r.campaign.id == campaignId) return r.campaign.isFavourite;
    }
    return false;
  }
  ...
  void openRewardedPost(RewardedPostEntity post) {
    // Campaign detail for rewarded posts is not ready yet.
  }
  ...
  void viewAllRecentlyRewarded() {
    Get.to(
      () => RecentlyRewardedListPage(
        ...
        // List is browse-only until detail is wired.
        onPostTap: null,
      ),
    );
  }
